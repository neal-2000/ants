...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place and current_place is not hive:
            if spaces >= self.min_range and spaces <= self.max_range and current_place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if amount >= self.armor:
            bees_on_place = self.place.bees[:]
            for bee in bees_on_place:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        #doubles ants behind it that have not already been doubled
        behind = self.place.exit
        while behind:
            if behind.ant:
                if not behind.ant.doubled:
                    behind.ant.damage *= 2
                    behind.ant.doubled = True
                if behind.ant.is_container:
                    if behind.ant.contained_ant and not behind.ant.contained_ant.doubled:
                        behind.ant.contained_ant.damage *= 2
                        behind.ant.contained_ant.doubled = True
            behind = behind.exit
...
def apply_effect(effect, bee, duration):
    ...
    def new_action(colony):
        nonlocal duration
        if effect == make_scare and not bee.scared_once:
            bee.scared = True
